...
async def get_calendar(request: CalendarRequest):
    ...
    output, err = run_command(
        os.path.abspath(os.path.join(os.sep, "usr", "bin", "osascript")),
        [os.path.join("scripts", "calendar_auth.scpt")],
    )
    if err:
...
def run_calendar_swift_script(payload: CalendarRequest):
    ...
    if not output:
        return (
            "No events found. Check that you are requesting the correct calendar.",
            None,
        )

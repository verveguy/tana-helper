...
class NodeIndex(BaseModel):
    ...
    def build_master_pairs(self):
        ...
        for node in self.tana_dump.docs:
                    ...
                    if node.props.ownerId and self.valid(node.props.ownerId):
                        ...
                        if not self.valid(data_node_id):
                            if not self.trashed(data_node_id):
                                logger.warning(
                                    f"Found tag tuple {node.id} with missing data node {data_node_id}"
                                )
                            ...
                            for tag_id in tag_ids:
                                ...
                                if self.valid(tag_id):
                                    if self.config.include_node_tag_links:
                                        self.master_pairs.append(
                                            (data_node_id, tag_id, IS_TAG_LINK)
                                        )
                                        # collect the tags...
            ...
            if (
                self.config.include_inline_refs
                and name
                and '<span data-inlineref-node="' in name
            ):
            ...
            if (
                self.config.include_content_nodes
                and node.children
                and "Root node for file:" not in name
                and "SYS" not in node.id
            ):
                for child_id in node.children:
                    if (
                        self.valid(child_id)
                        and SUPERTAG not in child_id
                        and FIELD not in child_id
                        and "SYS" not in child_id
                    ):
                        ...
                        if child_node.props.docType == "tuple":
                            ...
                            if child_node.children:
                                ...
                                if self.valid(field_id):
                                    node.fields.append(
                                        {"field": field_id, "values": value_ids}
                                    )
...
def add_linkage(
    index: NodeIndex, links: list, source_id: str, target_id: str, reason="unknown"
):

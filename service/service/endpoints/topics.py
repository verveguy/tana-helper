...
from service.tanaparser import (
    IS_CHILD_CONTENT_LINK,
    IS_TAG_LINK,
    NodeIndex,
    patch_node_name,
    prune_reference_nodes,
)
...
async def extract_topics(
    tana_dump: TanaDump, format: str = "TANA"
) -> list[TanaDocument]:
            ...
            for field_dict in node.fields:
                ...
                for value_id in value_ids:
                    if not index.valid(value_id):
                        logger.warning(
                            f"Invalid field value_id: {value_id} for field: {field_id}. Presumably trashed node."
                        )
                        continue
                    ...
                    if len(value_node.tags) > 0:
                        ...
                        value = (
                            "[["
                            + patch_node_name(index, value_id)
                            + "^"
                            + value_id
                            + "]]"
                            + add_tags(index, index.node(value_id).tags)
                        )
                    else:
            ...
            if len(content_node.tags) > 0:
                ...
                content.append(
                    (
                        ...
                        + content_id
                        + "]]"
                        + add_tags(index, content_node.tags),
                    )
                )
            else:

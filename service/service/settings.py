...
class Settings(BaseSettings):
    ...
    model_config = SettingsConfigDict(
        title="Settings", env_file=".env", env_file_encoding="utf-8"
    )
    ...
    tana_api_token: Annotated[
        str,
        Field(
            title="Tana API Token",
    ...
    webhook_template_path: Annotated[
        str,
        Field(
            title="Webhook Template Path", description="Path to store webhook templates"
        ),
    ] = os.path.join(Path.home(), ".tana_helper", "webhooks")

    temp_files: Annotated[
        str,
        Field(
            title="Temporary Files Path", description="Path to store temporary files"
        ),
    ] = os.path.join("/", "tmp", "tana_helper", "tmp")

    export_path: Annotated[
        str, Field(title="Export Path", description="Path to store exported files")
    ] = os.path.join("/", "tmp", "tana_helper", "export")

    tana_environment: Annotated[
        str,
        Field(
            title="Tana Pinecone Environment",
            description="Pinecone environment for Tana vector storage",
        ),
    ] = "us-west4-gcp-free"

    tana_namespace: Annotated[
        str,
        Field(
            title="Tana VectorDB Namespace",
            description="VectorDB namespace for Tana vector storage",
        ),
    ] = "tana-namespace"

    tana_index: Annotated[
        str,
        Field(
            title="Tana VectorDB Index",

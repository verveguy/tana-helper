...
@router.post(
    "/pinecone/upsert", status_code=status.HTTP_204_NO_CONTENT, tags=["Pinecone"]
)
    ...
    async with lock:
        ...
        vectors = [
            (
                req.nodeId,
                vector,
                {"category": TANA_NODE, "supertag": req.tags, "text": req.context},
            )
        ]
...
@router.post(
    "/pinecone/delete", status_code=status.HTTP_204_NO_CONTENT, tags=["Pinecone"]
)
...
def get_tana_nodes_for_query(req: PineconeRequest, send_text: bool | None = False):
    ...
    tag_filter = None
    if len(supertags) > 0:
        tag_filter = {"category": TANA_NODE, "supertag": {"$in": supertags}}
    ...
    if not send_text:
        ...
        best = filter(threshold_function, query_response.matches)
        docs = [
            {"sources": "[[^" + match.id + "]]", "answer": match.metadata["text"]}
            for match in best
        ]
        return docs
...
@router.post(
    "/pinecone/purge", status_code=status.HTTP_204_NO_CONTENT, tags=["Pinecone"]
)

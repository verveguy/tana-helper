...
def llama_ask_custom_pipeline(req: LlamaindexAsk, model: str):
    ...
    prevnext = WidenNodeWindowPostProcessor(
        storage_context=storage_context, num_nodes=5, mode="both"
    )
    summarizer = TreeSummarize(
        summary_template=summary_tmpl, service_context=service_context
    )
    ...
    for question in questions:
        ...
        research = "\n".join(
            [node.get_content(metadata_mode=MetadataMode.LLM) for node in new_nodes]
        )
        logger.info(f"Nodes:\n{research}")

        # tailor the summarizer prompt
        sum_result = summarizer.as_query_component().run_component(
            nodes=new_nodes, query_str=question
        )
        summary = sum_result["output"].response
        logger.info(f"Summary:\n{summary}")

        result = {"question": question, "answers": nodes, "summary": summary}

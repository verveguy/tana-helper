...
async def make_proxy_request(method: str, path: str, req: Request, body=None):
    ...
    if method == "POST":
        ...
        rp_resp = await client.post(url=target_url, headers=new_headers, json=body)
    elif method == "PUT":
        rp_resp = await client.put(url=target_url, headers=new_headers, json=body)
    elif method == "PATCH":
        rp_resp = await client.patch(url=target_url, headers=new_headers, json=body)
    elif method == "GET":
        rp_resp = await client.get(url=target_url, headers=new_headers)
    elif method == "DELETE":
        rp_resp = await client.delete(url=target_url, headers=new_headers)
    else:
        rp_resp = None

...
async def add_template(req: Request, schema: str, body: str = Body(...)):
    """
...
async def get_schemas():
    ...
    try:
        ...
        files = [
            f for f in directory if os.path.isfile(path + "/" + f)
...
async def get_template(schema: str):
    ...
    """
    try:
        with open(f"{path}/{schema}.jn2") as template_file:
...
async def add_schema(req: Request, schema: str, body: str = Body(...)):
    """
...
async def do_webhook(schema: str, body: str):
    ...
    try:
        ...
        completion_request = OpenAICompletion(
            prompt=prompt, max_tokens=1000, temperature=0, model="gpt-4"
        )
    ...
    if tana_result.is_error:
        logger.warning(
            f"Tana input API failed. {tana_result.status_code}. Detail: {tana_result.text}"
        )
        raise HTTPException(
            status_code=tana_result.status_code, detail=tana_result.text
        )

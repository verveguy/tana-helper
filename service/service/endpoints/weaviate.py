...
from service.dependencies import WeaviateRequest, get_embedding
...
def find_by_node_id(client, node_id):
    ...
    node_filter = {"path": ["nodeId"], "operator": "Equal", "valueString": node_id}
    result = (
        client.query.get("TanaNode", ["nodeId"])
        .with_additional("id")
...
@router.post(
    "/weaviate/upsert", status_code=status.HTTP_204_NO_CONTENT, tags=["Weaviate"]
)
async def weaviate_upsert(request: Request, req: WeaviateRequest):
    async with lock:
        ...
        metadata = {"nodeId": req.nodeId, "supertags": req.tags, "content": req.context}
        ...
        def do_upsert():
            ...
            if len(node_ids) > 1:
                # uh oh. dupes!
                logger.info(
                    f"DUPLICATES Found {len(node_ids)} nodes with nodeId={req.nodeId}"
                )
            ...
            if len(node_ids) > 0:
                ...
                client.data_object.create(
                    data_object=metadata, class_name="TanaNode", vector=vector
                )
...
@router.post(
    "/weaviate/delete", status_code=status.HTTP_204_NO_CONTENT, tags=["Weaviate"]
)
...
def get_tana_nodes_for_query(req: WeaviateRequest, send_text: bool | None = False):
    ...
    query = (
        client.query.get("TanaNode", ["nodeId", "content", "supertags"])
    ...
    if not send_text:
        ...
        best = filter(threshold_function, closest)
        docs = [
            {"sources": "[[^" + match["nodeId"] + "]]", "answer": match["content"]}
            for match in best
        ]
        return docs
...
@router.post(
    "/weaviate/purge", status_code=status.HTTP_204_NO_CONTENT, tags=["Weaviate"]
)

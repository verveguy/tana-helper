...
export default function RAGIndexControls() {
  const {obsidianExportData, setObsidianExportData, loading, setLoading} = useContext(TanaHelperContext)
  ...
  useEffect(() => {
    if (upload) {
      setLoading(true);
      setObsidianExportData(null);
      axios.post('/migrate/obsidian', dumpFile, {
        headers: {
          "Content-Type": "application/json",
        }
      })
        .then(response => {
          setObsidianExportData(response.data);
        })
        .catch(error => {
          console.error(error);
        })
